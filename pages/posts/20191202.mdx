...
を考える．ただし，$\phi_t$ は以下の関係を満たす凸関数系であり，実行可能集合を$X_t$と定義する．

$$
f(x)\leq t \Leftrightarrow \phi_t(x)\leq 0\ (t\in\mathrm{R})
$$

さらに，$\phi_t$ はパラメータ $t$ に関して非増加であるとする．すなわち，$\forall s,t,\ s\leq t\rightarrow\phi_s(x)\geq \phi_t(x)$ とする．

　値 $p^*$ を問題 ($\mathrm{P}$) の最適値とするとき，所与の $t$ に対して，($\mathrm{P}_F(t)$) が実行可能であれば，$p^*\leq t$ が成り立つが，実行可能でなければ $p^*\geq t$ となる．

　したがって，準凸最適化問題 ($\mathrm{P}$) を解く代わりに実行可能性問題 ($\mathrm{P}_F(t)$) を解くことで大域的最適解を求めることができる．
...
　このアルゴリズムは一見最適値を必要とするように見えるが，初期値 $l,\ u$ はそれぞれ，関数 $f$ の下界，下限より大きい値（下界に属さない値）を任意にとればいいので，$p^*$ を厳密に知る必要はない（というより，当然だが，$p^*$ がわかっていれば，問題を解く意味もほとんどなくなる）
...
\[BV2004\] Boyd, Stephen, and Lieven Vandenberghe. Convex optimization. *Cambridge university press*, 2004.
